 ...
 * 拉 GitHub 公开 API，取最近推送的自有仓库。
 *
 * 失败时不能让版式塌掉：容器高度由 CSS 的 aspect-ratio 撑着（与原贪吃蛇同比例），
 * 这里只负责换里面的内容，所以网络挂了也不会顶动页面。
 * ------------------------------------------------------------------------- */
const repos = ref([])
...
onMounted(async () => {
  try {
    const url = `https://api.github.com/users/${github.username}/repos?sort=pushed&per_page=100`
...
@media (max-width: 800px) {
  ...
  /* 窄屏下把时间压小一点，给仓库名让出宽度（否则长名字只剩省略号） */
  .recent-time {
    font-size: 11px;
    padding-left: 6px;
  }
